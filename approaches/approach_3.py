...
def run_simple_lstm_with_mda_pruning(ticker="AAPL", start_date="2015-01-01", end_date="2025-05-30", horizon=60):
    """
    Complete LSTM pipeline with MDA feature selection.
    """
    ...
    # =========================================================================
    st.header(" Preparing LSTM Training Data")
    ...
    with st.spinner("Creating sequences for LSTM..."):
    ...
    **Sequences Created for LSTM:**
    ...
    st.info(f"LSTM Input Shape: {input_shape}")
    ...
    st.header(" Intelligent Model Pruning & Analysis")
       # Define simple pruning function
    def simple_lstm_pruning(original_model, X_val=None, y_val=None, sensitivity=0.3):
        """Lightweight magnitude-based pruning for LSTM models"""
        ...
        for layer in pruned_model.layers:
            if isinstance(layer, layers.LSTM):
                ...
                if weights:
                    ...
                    threshold = np.percentile(np.abs(input_weights), sensitivity * 100)
                    mask = np.abs(input_weights) > threshold
                    weights[0] = input_weights * mask
                    layer.set_weights(weights)
        
        return pruned_model

    # Configure pruning parameters
    pruning_kwargs = {'sensitivity': 0.1}  # Prune 30% of smallest weights
    ...
    try:
        pruning_results = enhanced_pruning_with_comprehensive_visualization(
            ...
            y_true_orig=y_true,
            y_pred_orig=y_pred,
            pruning_function=simple_lstm_pruning,
    ...
    if pruning_success:
        ...
        ** Final Selected Features:** {', '.join(final_features)}
        ...
        st.warning(f"""
        ** LSTM Pipeline (Pruning Failed):**
        
         **Data:** {ticker} from {start_date} to {end_date}
         **Features:** {len(selected_features)} selected via MDA
         **Model:** LSTM with {input_shape} input shape
