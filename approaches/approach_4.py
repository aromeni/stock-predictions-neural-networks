...
def run_simple_gru_with_mda_pruning(ticker="AAPL", start_date="2015-01-01", end_date="2025-05-30", horizon=60):
    """
    Complete GRU pipeline with MDA feature selection.
    """
    ...
    # =========================================================================
    # STEP 5: PREPARE GRU TRAINING DATA
    # =========================================================================
    st.header(" Preparing GRU Training Data")
    ...
    with st.spinner("Creating sequences for GRU..."):
    ...
    **Sequences Created for GRU:**
    ...
    # STEP 6: GRU MODEL
    # =========================================================================
    st.header(" GRU Model Architecture")
    
    input_shape = (X_train.shape[1], X_train.shape[2])
    st.info(f"GRU Input Shape: {input_shape}")

    with st.spinner("Building GRU model..."):
        model = build_gru_model(input_shape)

    st.write("### GRU Model Summary")
    ...
    def simple_gru_pruning(original_model, X_val=None, y_val=None, input_sensitivity=0.3, rec_sensitivity=0.2):
        """
        Enhanced GRU pruning with separate sensitivity for input/recurrent weights
        """
        ...
        for layer in pruned_model.layers:
            if isinstance(layer, layers.GRU):
                ...
                # 1. Prune input weights (less sensitive)
                if weights[0] is not None:
                    input_weights = weights[0]
                    input_threshold = np.percentile(np.abs(input_weights), input_sensitivity * 100)
                    input_mask = np.abs(input_weights) > input_threshold
                    weights[0] = input_weights * input_mask
                    
                # 2. Prune recurrent weights (more conservative)
                if len(weights) > 1 and weights[1] is not None:
                    rec_weights = weights[1]
                    
                    # Gate-aware pruning - focus on less important reset gate
                    units = rec_weights.shape[0]
                    reset_gate = rec_weights[:, :units]  # First third: reset gate
                    update_gate = rec_weights[:, units:2*units]  # Middle third: update gate
                    candidate_gate = rec_weights[:, 2*units:]  # Last third: candidate
                    
                    # Conservative reset gate pruning
                    reset_threshold = np.percentile(np.abs(reset_gate), rec_sensitivity * 100)
                    reset_mask = np.abs(reset_gate) > reset_threshold
                    reset_gate = reset_gate * reset_mask
                    
                    # Recombine gates
                    rec_weights = np.concatenate([reset_gate, update_gate, candidate_gate], axis=1)
                    weights[1] = rec_weights
    ...
    pruning_kwargs = {'input_sensitivity': 0.1, # Prune 30% of smallest weights
                      'rec_sensitivity': 0.15  # Extra conservative for recurrent weights
                      }  
    ...
    try:
        pruning_results = enhanced_pruning_with_comprehensive_visualization(
            ...
            y_true_orig=y_true,
            y_pred_orig=y_pred,
            pruning_function=simple_gru_pruning,
    ...
    if pruning_success:
        ...
        ** Final Selected Features:** {', '.join(final_features)}
        ...
        st.warning(f"""
        ** GRU Pipeline (Pruning Failed):**
        
         **Data:** {ticker} from {start_date} to {end_date}
         **Features:** {len(selected_features)} selected via MDA
         **Model:** GRU with {input_shape} input shape
